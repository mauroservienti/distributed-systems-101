...
(async () => {
    ...
    const inputQ = "request-multi-response-sender-endpoint";
    const destinationQ = "request-multi-response-receiver-endpoint";
    ...
    channel.consume(inputQ, (msg) => {
        if (msg !== null) {
            console.log(`- [x] Received '${msg.content.toString()}'. Correlation ID '${msg.properties.correlationId}'`);
            channel.ack(msg);
        } else {
    ...
    const sendResult = await channel.sendToQueue(destinationQ, Buffer.from(msg),{
        persistent: true,
        replyTo: inputQ,
        correlationId: "some-unique-value-to-this-message-eg-order-id"
    });
